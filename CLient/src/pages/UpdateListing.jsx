...
//             <h1 className="text-3xl font-bold text-gray-600 text-center my-7">Update Listing</h1>
            
//             {/* Success Notification */}
//             {success && (
//                 <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4 text-center">
//                     <p className="font-semibold">🎉 Listing updated successfully!</p>
...
const Create_Listing = () => {
  ...
  const storeImage = async (file, index) => {
    return new Promise((resolve, reject) => {
      ...
      xhr.onerror = function () {
        reject(new Error('Network error - check server connection'));
      };
  ...
  const handleImageSubmit = () => {
    if (files.length > 0 && files.length + formData.imageUrls.length <= 6) {
      ...
      const promises = Array.from(files).map((file, i) => storeImage(file, i));
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <textarea
            ...
            id="description"
            required
            className="border p-3 h-64 rounded-lg"
            onChange={handleChange}
            value={formData.description}
            disabled={loading}
          />
          ...
          <div className="flex flex-wrap gap-6">
            <div className="flex items-center gap-2">
              <input type="number" id="bedrooms" min="1" max="10" required onChange={handleChange} value={formData.bedrooms} disabled={loading} />
              <p>Beds</p>
            </div>
            <div className="flex items-center gap-2">
              <input type="number" id="bathrooms" min="1" max="10" required onChange={handleChange} value={formData.bathrooms} disabled={loading} />
              <p>Baths</p>
            </div>
            <div className="flex items-center gap-2">
              <input type="number" id="regularPrice" min="50" max="10000000" required onChange={handleChange} value={formData.regularPrice} disabled={loading} />
              <div className="flex flex-col items-center">
                <p>Regular price</p>
                {formData.type === 'rent' && <span className="text-xs">(Naira / month)</span>}
              </div>
            </div>
            {formData.offer && (
              <div className="flex items-center gap-2">
                <input type="number" id="discountedPrice" min="0" max="10000000" required onChange={handleChange} value={formData.discountedPrice} disabled={loading} />
                <div className="flex flex-col items-center">
                  <p>Discounted price</p>
                  {formData.type === 'rent' && <span className="text-xs">(Naira / month)</span>}
                </div>
        ...
        {/* Right side - Images */}
        <div className="flex flex-col flex-1 gap-4">
          <p className="font-semibold">Images (max 6, first is cover)</p>
          <div className="flex gap-4">
            <input onChange={(e) => setFiles(e.target.files)} type="file" accept="image/*" multiple className="border p-3 rounded w-full" disabled={loading} />
            <button type="button" disabled={uploading || loading} onClick={handleImageSubmit} className="p-3 border text-green-700 border-green-700 rounded">
              {uploading ? 'Uploading...' : 'Upload'}
            </button>
          </div>
          ...
          {formData.imageUrls.map((url, index) => (
            <div key={url} className="flex justify-between p-3 border items-center">
              ...
              <button type="button" onClick={() => handleRemoveImage(index)} className="p-3 text-red-700">Delete</button>
            </div>
          ))}
          ...
          {!loading && !success && (
            <button type="button" onClick={resetForm} className="p-3 bg-gray-500 text-white rounded-lg">
              Reset Form
            </button>
          )}
        </div>
